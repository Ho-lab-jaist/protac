...
class ProTacVelocityController(Node):
    def __init__(self):
        ...
        ps = np.array([0.3, -0.2, 0.4])
        pe = np.array([0.3, 0.2, 0.4])
        Tf = 4
        self.trajectory_generation = self.jonit_trajectory_generation(ps, pe, Tf)

        self.contact = False

    def jonit_trajectory_generation(self, pointstart, pointend, Tf):
        N = Tf/self.ts + 1.0
        ...
        while True:
            for pos, posdot in zip(traj, trajdot):
                joint = self.kinematics.IKinPos(pos)[0]
                Rsb = self.kinematics.FKinBody(joint)[:3,:3]
                Jb = self.kinematics.JacobianBody(joint)[3:,:]
                Ja = np.dot(Rsb, Jb)
                jointdot = np.dot(np.linalg.pinv(Ja), posdot)
                yield jointdot
    ...
    def timer_callback(self):   
        ...
        if self.contact:
            ...
            joint_velocity_cmd = next(self.trajectory_generation)

        msg.data = [joint_velocity_cmd[0], joint_velocity_cmd[1], joint_velocity_cmd[2], joint_velocity_cmd[3]]
...
def main(args=None):
    ...
    try:
        ...
        print('Node stopped cleanly: Stop robot motion')
        for i in range(5):
            msg = Float64MultiArray()
            msg.data = [0., 0., 0., 0.]
            velocity_controller.get_logger().info('Publishing: "%s"' % msg.data)
            velocity_controller.publisher_.publish(msg)
    finally:
