...
def map_area_to_distance(input_value, 
                            input_min = 8000, 
                            input_max = 90000, 
                            output_min = 4, 
                            output_max = 0):
...
def draw_boxes(
    img, bbox_tlbr, risk_score, class_prob=None, class_idx=None, class_names=None
):
    """
    ...
    #         distance = map_area_to_distance(area)
    #         # bbox_text.append("({:.2f})".format(class_prob[i]))
    #         bbox_text.append("({:.2f})".format(distance))
    ...
    for i, (tl_x, tl_y, br_x, br_y) in enumerate(bbox_tlbr):
        if class_idx[i]==0:
            ...
            if class_prob is not None:
                ...
                # distance = map_area_to_distance(area)
                # bbox_text.append("({:.2f})".format(class_prob[i]))
                bbox_text.append("({:.2f})".format(risk_score))
...
def inference(
    net, images, device="cuda", prob_thresh=0.5, nms_iou_thresh=0.3,
    resize=True
):
    """
